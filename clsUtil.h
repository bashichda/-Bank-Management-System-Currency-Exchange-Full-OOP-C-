...
public:
    ...
    static void Swap(clsDate& A, clsDate& B) { clsDate Temp = A; A = B; B = Temp; }
    ...
    static string EncryptText(string Text, short EncryptionKey = 17)
    {
    ...
    static string DecryptText(string Text, short EncryptionKey = 17)
    {
        for (int i = 0; i < Text.length(); i++)
        {
            Text[i] = char((int)Text[i] - EncryptionKey);
